        ...
        await firestore()
            ...
            .then(docs => {
                ...
                docs.forEach(async doc => {
                    ...
                    const post = await firestore().collection('Post').doc(doc.id).collection('posts').where('status', '==', 0).get()
                    ...
                    // allposts.map(res => data1.push(res))
                    // firestore().collection('Post').doc(doc.id).collection('posts').get().then(
                    //     post2 => {
                    //         post2.forEach(doc2 => {
                    //            doc2.id=
                    //         })
        ...
        <View
            ...
            <FlatList
                ListHeaderComponent={() => <View>
                    <HeaderCustomBot title='Bài đăng chờ duyệt' back={() => navigation.pop()} />
